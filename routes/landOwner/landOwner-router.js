  ...
  LandOwners.findById(id)
    .then(listing => {
      if (listing) {
        ...
        res.status(404).json({
          message: "Could not get that listing..."
        })
      }
    }).catch(err => {
      console.log(err)
...
router.post('/', (req, res) => {
  ...
  LandOwners.addlandOwner(landOwner)
    ...
    .catch(error => {
      res.status(500).json({
        message: 'error saving landOwner information'
      });
    });
});
...
router.put('/:id', (req, res) => {
  ...
  LandOwners.updateLandOwner(req.params.id, changes)
    ...
    .catch(error => {
      res.status(500).json({
        message: 'error getting the landOwners data'
      })
    })
})

// took it out of PUT of put request...
router.delete('/:id', (req, res) => {
  let id = req.params.id;
  console.log(id)
  LandOwners.removeLandOwner(id)
    .then(landOwner => {
      if (landOwner) {
        res.status(201).json(landOwner)
      } else {
        res.status(404).json({
          message: 'LandOwner not found'
        })
      }
    })
    .catch(error => {
